...
negNets <-
        function(input,
                ...
                haters <-
                        users %>% filter(category == 'N', favGroup == select_parameters$favGroup) %>%
                        arrange(eval(con22), eval(con23)) %>%
                        head(select_parameters$numberHaters)
                ...
                nets <- apply(haters, 1, function(hater) {
                        ...
                        value_days <-
                                sapply(value_sens, function(vs_counter) {
                                        if (vs_counter > 0)
                                                2:(vs_counter + 1)
                                }) %>% unlist()
                        
                        layout <- data.frame(Date = as.Date(c(
                                user_sentiments$day, days
                        )),
                        value = c(value_days, rep(1, length(
                                days
                        ))))
...
negSensTimeline <-
        function(input, output, session, nets, current_week) {
                days <- current_week
                ...
                lapply(seq_along(nets), function(i) {
                        ...
                        output[[paste0('timeGraph', i)]] <- renderPlot({
                                ...
                                V(net)$size <-
                                        ifelse(
                                                ...
                                                ifelse(
                                                        V(net)$favGroup == 'low',
                                                        11,
                                                        ifelse(V(net)$favGroup ==
                                                                       'popular', 14, 18)
                                                )
                                        )
                                
                                V(net)$color <-
                                        attitudePalette[V(net)$attitude]
                                V(net)$label = NA
                                V(net)$label.font = 2.5
                                V(net)$label.color = V(net)$color
                                V(net)[is.na(V(net)$attitude)]$size <- 15
                                V(net)[is.na(V(net)$attitude)]$color <-
                                        '#9B97D9'
                                V(net)$label.cex = .9
                                
                                E(net)$color <-
                                        ifelse(E(net)$nhates > 0,
                                               '#F2134F',
                                               '#AAAAAA')
                                E(net)$width = 1
                                E(net)$curved = 0.35
                                E(net)$label <-
                                        ifelse(E(net)$nhates > 0, E(net)$nhates, NA)
                                E(net)$arrow.size <- .5
                                E(net)$arrow.width <- .5
                                ...
                                plot.igraph(
                                        net,
                                        layout = layout,
                                        xlim = as.Date(c(days[1], days[length(days)] +
                                                                 1)),
                                ...
                                axis(
                                        1,
                                        at = as.numeric(layout$Date),
                                        labels = layout$Date,
                                        cex.axis = 0.9
                                )
                                axis(
                                        2,
                                        at = 1:max(layout$value),
                                        labels = 1:max(layout$value)
                                )
                        ...
                        output[[paste0('timeDegree', i)]] <- renderPlot({
                                ...
                                E(net)$weight <- E(net)$nhates
                                deg <-
                                        data.frame(value = degree(net, v = V(net)[V(net)$name %in% as.character(days)]))
                        ...
                        output[[paste0('timeStrength', i)]] <- renderPlot({
                                ...
                                E(net)$weight <- E(net)$nhates
                                str <-
                                        data.frame(value = strength(net, v = V(net)[V(net)$name %in% as.character(days)]))
